      ...
      begin
        ...
        doc.css("tr").each do |content|
          if content.at_css("td")
            ...
            county = content.at_css("td:last a").text.gsub(/^[a-z]*-/i, '').gsub(/( Borough)?( Parish)?( County)?(, City of Boston)?/, '')
            location.counties << County.find_by_name(county) if state == location.state
          end
        end
        location.save
      rescue
